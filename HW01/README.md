...
You need to understand that computers are "designed to be
completely stupid".  Computers were invented to control weapons and
computer must do precisely what programmers instruct to do.  Computers
do not understand "small" mistakes or "big" mistakes. For computers,
...
positions.  ECE 264 is not designed for the general public and it is
not an introductory course.
ECE 264 emphasizes understanding and discourages *memorization without
understanding*.
...
You can find  the electronic version of Dr. Lu's book
(Intermediate C Programming by CRC Press,
https://www.safaribooksonline.com/library/view/intermediate-c-programming/9781498711647/?ar).  Please go to Purdue
Library web and search the book.  If you prefer a physical copy,
please go to WALC to check it out.  You should read the book before
attempting to write any code.  You can save a lot of time by reading
the book and learning what is correct without making common mistakes.
You should also attend *every* lecture because the instructors have
seen many common mistakes and will tell students how to avoid these
mistakes.
...
learning these tools takes time, these tools can save
you much more time. Some students skip learning these tools hoping to
save time, but they spend much more time debugging their programs.
...
Since you are advise to use version control, the instructors will not
...
If you do not understand what *path* means, that's all right for
now. 
...
implement functions. Header files are "included" using

`#include "header.h"`

If `header.h` is created by you. If a header file is available from the
C language, use

`#include <header.h>`

Source files are "compiled" and "linked".  To link source files,
simply put the list of source (.c) files after `gcc`.
...
```
DEBUGFLAGS = -DTEST_FUNC1 -DTEST_FUNC2 -DTEST_FUNC2
CFLAGS = -std=c99 -g -Wall -Wshadow --pedantic -Wvla -Werror
GCC = gcc $(CFLAGS) $(DEBUGFLAGS)
```
This is equivalent to

`GCC = gcc -std=c99 -g -Wall -Wshadow --pedantic -Wvla -Werror -DTEST_FUNC1 -DTEST_FUNC2 -DTEST_FUNC2`

**Warning** Your learning experience to great extend depends on
...
`assert` because the programmer *think* it should be right but is not
...
ECE 264 is completely different from CS 159. Comparing these two makes
no sense and would not help you.  

ECE 264 is about 1,000 times more difficult than CS 159.  That is the reason
